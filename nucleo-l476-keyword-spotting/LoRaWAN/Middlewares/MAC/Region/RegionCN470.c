...
void RegionCN470InitDefaults( InitDefaultsParams_t* params )
{
    ...
    switch( params->Type )
    {
        case INIT_TYPE_DEFAULTS:
        {
            ...
            NvmCtx.ChannelsDefaultMask[3] = 0x0000;
            NvmCtx.ChannelsDefaultMask[4] = 0x0000;
            NvmCtx.ChannelsDefaultMask[5] = 0xFF00;
			...
			NvmCtx.ChannelsDefaultMask[4] = 0x0000;
			NvmCtx.ChannelsDefaultMask[5] = 0x0000;
			NvmCtx.ChannelsDefaultMask[6] = 0x0001;*/
...
void RegionCN470ApplyCFList( ApplyCFListParams_t* applyCFList )
{
    ...
    // ChMask0 - ChMask5 must be set (every ChMask has 16 bit) jxr 
    for( uint8_t chMaskItr = 0, cntPayload = 0; chMaskItr <= 6; chMaskItr++, cntPayload+=2 )
    {
...
bool RegionCN470ChanMaskSet( ChanMaskSetParams_t* chanMaskSet )
{
    switch( chanMaskSet->ChannelsMaskType )
    {
        case CHANNELS_MASK:
        {	/* jxr*/
            RegionCommonChanMaskCopy( NvmCtx.ChannelsMask, chanMaskSet->ChannelsMaskIn, 7 );
            break;
        }
        case CHANNELS_DEFAULT_MASK:
        {
            RegionCommonChanMaskCopy( NvmCtx.ChannelsDefaultMask, chanMaskSet->ChannelsMaskIn, 7 );
            break;
        }
        default:
            return false;
...
uint8_t RegionCN470LinkAdrReq( LinkAdrReqParams_t* linkAdrReq, int8_t* drOut, int8_t* txPowOut, uint8_t* nbRepOut, uint8_t* nbBytesParsed )
{
	...
	/*jxr configuration*/
    uint16_t channelsMask[7] = { 0, 0, 0, 0, 0, 0, 0};
    GetPhyParams_t getPhy;
    PhyParam_t phyParam;
    RegionCommonLinkAdrReqVerifyParams_t linkAdrVerifyParams;

    // Initialize local copy of channels mask
    RegionCommonChanMaskCopy( channelsMask, NvmCtx.ChannelsMask, 7 );
    ...
    while( bytesProcessed < linkAdrReq->PayloadSize )
    {
        ...
        {
            // Enable all 125 kHz channels jxr
    ...
    {
        // Copy Mask
        RegionCommonChanMaskCopy( NvmCtx.ChannelsMask, channelsMask, 7 );
    }
...
LoRaMacStatus_t RegionCN470NextChannel( NextChanParams_t* nextChanParams, uint8_t* channel, TimerTime_t* time, TimerTime_t* aggregatedTimeOff )
{
    ...
    { // Reactivate default channels
		...
		//NvmCtx.ChannelsMask[6] = 0xFFFF;
		NvmCtx.ChannelsMask[0] = 0x0000;
        NvmCtx.ChannelsMask[1] = 0x0000;
        NvmCtx.ChannelsMask[2] = 0x0000;
        NvmCtx.ChannelsMask[3] = 0x0000;
        NvmCtx.ChannelsMask[4] = 0x0000;
        NvmCtx.ChannelsMask[5] = 0xFF00;
		//NvmCtx.ChannelsMask[6] = 0x0001;
    }
